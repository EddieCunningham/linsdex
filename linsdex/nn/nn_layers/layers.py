...
import linsdex.nn.nn_layers.util as util
import einops
...
class WeightNormDense(AbstractBatchableObject):
  ...
  def batch_size(self) -> Union[None,int,Tuple[int]]:
    if self.W.ndim == 2:
      ...
      raise ValueError(f"Invalid number of dimensions: {self.W.ndim}")
...
class WeightNormConv(AbstractBatchableObject):
  ...
  @property
  def batch_size(self) -> Union[None,int,Tuple[int]]:
    if self.W.ndim == 4:  # (filter_h, filter_w, in_channels, out_channels)
      return None
    elif self.W.ndim == 5:
      return self.W.shape[0]
    elif self.W.ndim > 5:
      return self.W.shape[:-4]
    else:
      raise ValueError(f"Invalid number of dimensions: {self.W.ndim}")
...
class WeightStandardizedConv(AbstractBatchableObject):
...
class ChannelConvention(AbstractBatchableObject):
  module: AbstractBatchableObject
  def __init__(self, module: AbstractBatchableObject):
    super().__init__()
    self.module = module

  @property
  def batch_size(self) -> Union[None,int,Tuple[int]]:
    return self.module.batch_size
...
class ConvAndGroupNorm(AbstractBatchableObject):
...
class Upsample(AbstractBatchableObject):
...
class Downsample(AbstractBatchableObject):
...
class GatedGlobalContext(AbstractBatchableObject):
...
class Attention(AbstractBatchableObject):
  ...
  @property
  def batch_size(self) -> Union[None,int,Tuple[int]]:
    return self.conv_in.batch_size
...
class LinearAttention(AbstractBatchableObject):
...
class AttentionBlock(AbstractBatchableObject):
