...
class GatedResBlock(AbstractBatchableObject):
  ...
  def batch_size(self) -> Union[None,int,Tuple[int]]:
    return self.linear1.batch_size
  ...
  def data_dependent_init(self,
    ...
    **Returns**:
    ...
    if y is not None:
      ...
      # For 3D inputs with 1D conditioning, reshape shift and scale to allow broadcasting with spatial dimensions
      if len(self.input_shape) == 3 and len(self.cond_shape) == 1:
        shift = shift[:, None, None, :]  # (batch, features) -> (batch, 1, 1, features)
        scale = scale[:, None, None, :]  # (batch, features) -> (batch, 1, 1, features)
    else:
      linear_cond = None
  ...
  def __call__(self, x: Array, y: Array = None) -> Array:
    ...
    **Returns:**
    ...
    if y is not None:
      ...
      # For 3D inputs with 1D conditioning, reshape shift and scale to allow broadcasting with spatial dimensions
      if len(self.input_shape) == 3 and len(self.cond_shape) == 1:
        shift = shift[None, None, :]  # (features,) -> (1, 1, features)
        scale = scale[None, None, :]  # (features,) -> (1, 1, features)
...
class Block(AbstractBatchableObject):
  ...
  def __call__(self,
               x: Array,
               y: Array = None,
               shift_scale: Optional[Tuple[Array, Array]] = None) -> Array:
    assert x.shape == self.input_shape, 'Only works on unbatched data'
...
class ImageResBlock(AbstractBatchableObject):
